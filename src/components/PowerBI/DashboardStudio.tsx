...
export const DashboardStudio: React.FC<DashboardStudioProps> = ({ sheet, onClose, onLoadSampleSales }) => {
  ...
  const kpis = useMemo(() => [
    { title: `Total ${activeMetricProfile?.colName || 'Valor'}`, value: formatMetricValue(totalSum), icon: <DollarSign className="size-4 text-emerald-600" /> },
            ...
            <div className="mt-6 flex flex-col gap-2">
              {onLoadSampleSales && (
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-xl bg-emerald-600 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-emerald-700 transition-all"
                >
              ...
              <button
                onClick={onClose}
                className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-all"
              >
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-slate-700">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-800 focus:border-emerald-500 focus:outline-none"
                >
            ...
            {kpis.map((kpi, idx) => (
              ...
              >
                <div>
                  ...
                  <div className="mt-1 text-lg font-bold text-slate-800 font-mono">{kpi.value}</div>
                </div>
                <div className="flex size-9 items-center justify-center rounded-xl bg-slate-50 border border-slate-100">
                  {kpi.icon}
                </div>
              </div>
            ))}
